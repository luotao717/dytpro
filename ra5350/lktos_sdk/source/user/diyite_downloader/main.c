...
inline int apk_info_copy(struct apk_info *dest, struct apk_info *src)
{
	...
	/*添加代码解决编号重复问题*/
	struct apk_info *tempapk = (struct apk_info*)soft_list.base;
    struct apk_info *apkseq = (struct apk_info*)soft_list.base;
        
    int i=0;
    int num[512]={0};
        
    for(i=0;apkseq && i<soft_list.nmember;apkseq++,i++)
        num[apkseq->seq] = apkseq->seq;
    for(i=0;tempapk && i<soft_list.nmember;tempapk++,i++)
    {
        if(tempapk->seq == dest->seq && strcmp(tempapk->id,dest->id))
        {       
            int j=1;
            for(j=1;j<=soft_list.nmember;j++)
            {
                if(num[j] !=j)
	            {
                    tempapk->seq = j;
		...
		if (apk_list_ptr[soft_list_index].update_time < timestamp)
		{
			...
			if (soft_list_index != soft_list.nmember - 1)
			{
				memcpy(&apk_list_ptr[soft_list_index], 
						...
						/*下面自己添加，解决更新程序退出问题*/
				memset(&apk_list_ptr[soft_list.nmember - 1],0,soft_list.size);		
			}
			--soft_list.nmember;
			--soft_list_index;
		}
